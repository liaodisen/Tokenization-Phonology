...
def require_cuda():
    if not torch.cuda.is_available():
        raise RuntimeError(
            "CUDA is required for evaluate_math_chat.py, but no CUDA GPU is available."
        )
...
def generate(model, tokenizer, input_text, generate_kwargs):
    ...
    input_ids = input_text.input_ids.to(model_device)
    attention_mask = input_text.attention_mask.to(model_device)
...
def main():
    ...
    if args.model == "llama3.1":
        ...
        args.model_name_or_path = "/model-weights/Meta-Llama-3.1-8B-Instruct"
    ...
    # Add argument parsing for n_shot
    n_shot = args.n_shot if hasattr(args, 'n_shot') else N_SHOT
    print(f"n_shot: {n_shot}")
    cot_flag = args.cot_flag if hasattr(args, 'cot_flag') else COT_FLAG
    ...
    if args.use_lora:
        results_path = os.path.join(args.output_dir, "results_lora.txt")
        scores_path = os.path.join(args.output_dir, "scores_lora.txt")
    else:
