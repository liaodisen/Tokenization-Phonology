...
def load_embeddings_pkl(file_path):
    ...
    syllables = data['syllable_count']
    return word, embeddings, syllables

class MLPClassifier(nn.Module):
    def __init__(self, input_size, hidden_size, output_size=700, num_classes_per_output=5):
    ...
    for i in range(layers):
        ...
        for seed in range(10, 20):
            ...
            if args.control_task_label:
                label_bad = np.random.randint(0, 5, size=len(label_bad))
                label_good = np.random.randint(0, 5, size=len(label_good))
